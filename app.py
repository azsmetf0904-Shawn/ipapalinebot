...
def init_db():
    conn = get_db()
    conn.executescript("""
        CREATE TABLE IF NOT EXISTS groups (group_id TEXT PRIMARY KEY, joined_at TEXT NOT NULL);
...
def upload_image_to_imgbb(image_data, filename="image.jpg"):
    ...
    try:
        ...
        r = requests.post("https://api.imgbb.com/1/upload", data={"key": IMGBB_API_KEY, "image": b64, "name": filename})
        data = r.json()
        if data.get("success"):
...
@app.route("/")
def index():
    return f'LINE 公告機器人 ✅ | 群組:{len(get_all_group_ids())} | <a href="/admin">管理後台</a>'

@app.route("/admin/timezone", methods=["GET"])
def get_timezone():
    if not check_admin(request): return jsonify({"error":"unauthorized"}),401
    return jsonify({"timezone": TIMEZONE, "current_time": now_local().strftime("%Y-%m-%d %H:%M:%S")})
...
def set_timezone():
    ...
    global TIMEZONE
    TIMEZONE = tz
    return jsonify({"ok":True,"timezone":tz,"current_time":now_local().strftime("%Y-%m-%d %H:%M:%S")})

# 其他 Admin 路由（簡化列出重要部分）
...
def ai_parse_course():
    if not check_admin(request): return jsonify({"error":"unauthorized"}),401
    return jsonify({"ok":False, "error":"目前未設定 AI 金鑰，請使用後台手動新增課程"})
...
def handle_text(event):
    ...
    if text.startswith("/公告 "):
        ok, total = push_to_groups([{"type":"text","text": f"📢 {text[4:].strip()}"}])
        ...
        try:
            ...
            title = parts[0]
            course_date = parts[1] if len(parts) > 1 else ""
            course_time = parts[2] if len(parts) > 2 else "09:00"
            location = parts[3] if len(parts) > 3 else ""
            description = parts[4] if len(parts) > 4 else ""

            if not title or not course_date:
                raise ValueError("缺少課程名稱或日期")
            ...
            cur = conn.execute(
                ...
                (title, course_date, course_time, location, description, "", now_local().isoformat())
            )
            cid = cur.lastrowid
            conn.commit()
            conn.close()

            dates = generate_reminders(cid, course_date, 30, "days", 7, "days")
            ...
            reply_message(reply_token, 
                ...
                "/加課 課程名稱 | 日期 | 時間 | 地點 | 說明\n\n"
                "範例：\n"
                "/加課 Python 入門 | 2026-06-20 | 14:00 | 線上 | 基礎教學")

    elif text == "/課程清單":
        conn = get_db()
        rows = conn.execute("SELECT title, course_date FROM courses ORDER BY course_date ASC LIMIT 15").fetchall()
        conn.close()
        if not rows:
            reply_message(reply_token, "目前沒有課程")
        else:
            msg = "📋 課程清單：\n" + "\n".join(f"📅 {r['course_date']} {r['title']}" for r in rows)
            reply_message(reply_token, msg)

    elif text == "/群組清單":
        reply_message(reply_token, f"目前已連接 {len(get_all_group_ids())} 個群組")
